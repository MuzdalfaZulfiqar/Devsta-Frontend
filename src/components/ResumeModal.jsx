...
import { getResumeTemplates } from "../api/resumeTemplates"; // ✅ templates API
...
export default function ResumeModal({ userId, isOpen, onClose, onSaveSuccess }) {
  ...
  // Reset state when closing modal so reopening refetches fresh data
  useEffect(() => {
    if (!isOpen) {
      setDraft(null);
      setError("");
      ...
      setSaving(false);
      setMessageIndex(0);
      // keep templateId as last chosen (don’t reset)
    }
  }, [isOpen]);
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await axios.post(
        ...
        {
          userId, // backend uses token as source of truth; okay to send
          draftProfile,
          templateId, // ✅ IMPORTANT: send selected template
        },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const toggleInclude = (key, idx) => {
    setDraft((prev) => ({
      ...prev,
      [key]: prev[key].map((item, i) => (i === idx ? { ...item, include: !item.include } : item)),
    }));
  };
          ...
          <div className="p-0">
            <div className="relative h-[420px] bg-gray-50 overflow-hidden rounded-b-2xl">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center z-10 px-8 text-center">
                ...
                <p className="mt-6 text-sm text-gray-500">This usually takes 8–15 seconds...</p>
              </div>
            </div>
          </div>
        ) : (
              ...
              <section>
                ...
                <div className="space-y-4">
                  {draft.education.map((edu, idx) => (
                    <div
                      key={idx}
                      className="p-5 border rounded-xl bg-gray-50/40 hover:bg-gray-50 transition"
                    >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateArrayItem("education", idx, "marksPercent", Number(e.target.value) || null)
                            }
                      ...
                      <div className="grid grid-cols-2 gap-4 mt-4">
                        <div>
                          ...
                          <input
                            ...
                            value={edu.startYear || ""}
                            onChange={(e) =>
                              updateArrayItem("education", idx, "startYear", e.target.value ? Number(e.target.value) : "")
                            }
                            placeholder="2018"
                            className="w-full px-4 py-2 border rounded-lg"
                          />
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={edu.endYear || ""}
                            onChange={(e) =>
                              updateArrayItem("education", idx, "endYear", e.target.value ? Number(e.target.value) : "")
                            }
                            placeholder="2022"
                            className="w-full px-4 py-2 border rounded-lg"
                          />
                    ...
                    {draft.githubRepos.map((repo, idx) => (
                      <div
                        ...
                        className={`p-4 border rounded-xl transition ${
                          repo.include ? "bg-blue-50 border-blue-300" : "hover:bg-gray-50"
                        }`}
                      >
        ...
        <div className="px-6 py-5 border-t bg-gray-50 flex items-center justify-end gap-4 rounded-b-2xl">
          ...
          <button
            ...
            className={`
              ...
              ${saving || loading || !draft ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700 shadow-sm"}
            `}
          >
