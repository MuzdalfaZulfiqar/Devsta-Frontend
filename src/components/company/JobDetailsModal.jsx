...
export default function JobDetailsModal({
  ...
  // ── Local applied state — updates immediately, no reload needed ──
  ...
  // Called when ApplyJobModal successfully submits
  const handleApplied = () => {
    setHasApplied(true);         // instant UI update inside this modal
    onApplied?.(jobId);          // notify AllJobsTab to update the card
  ...
  const timeAgo = (date) => {
    ...
    const parsed = new Date(date);
    if (isNaN(parsed.getTime())) return "—";
    const diffDays = Math.floor((new Date() - parsed) / (1000 * 60 * 60 * 24));
    if (diffDays <= 0) return "Today";
    if (diffDays === 1) return "Yesterday";
    if (diffDays < 7) return `${diffDays}d ago`;
    if (diffDays < 30) return `${Math.floor(diffDays / 7)}w ago`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white w-full max-w-3xl max-h-[92vh] rounded-2xl shadow-2xl overflow-hidden border border-gray-200 flex flex-col">
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 p-2 rounded-full bg-white shadow-sm hover:bg-gray-100 transition-colors"
              ...
              {job.requiredSkills?.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {job.requiredSkills.map((skill) => (
                    <span
                      key={skill}
                      className="px-3 py-1 text-sm font-medium border border-primary/40 text-primary bg-primary/5 rounded-full"
                    >
                      {skill}
                    </span>
                  ))}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {job.benefits.map((benefit, i) => (
                        <div
                          key={i}
                          className="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-lg border border-gray-200 text-gray-800 text-sm"
                        >
                          <div className="w-2 h-2 rounded-full bg-primary flex-shrink-0" />
                          {benefit}
                        </div>
        ...
        <div className="px-6 py-5 border-t bg-gray-50 flex flex-col sm:flex-row gap-3">
          {job && context === "browse" && (
            <>
              {hasApplied ? (
                // ── Applied state — shows immediately after submit ──
                <div className="flex-1 px-6 py-3 bg-green-50 text-green-700 border border-green-200 rounded-lg text-center font-medium flex items-center justify-center gap-2">
          ...
          <button
            onClick={() => onViewCompany?.(job?.company)}
            disabled={!job?.company}
            className="flex-1 sm:flex-none px-6 py-3 border border-gray-300 text-gray-800 font-medium rounded-lg hover:bg-gray-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            View Company
          </button>
        </div>
      ...
      {/* Apply modal rendered outside the scrollable area */}
      {showApplyModal && job && (
        <ApplyJobModal
          job={job}
          onClose={() => setShowApplyModal(false)}
