...
export default function AllJobsTab({
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        // Fetch more jobs so All Jobs tab shows all ranked active jobs
        const data = await getRecommendedJobs({ k: 200 }, token);
        const fetched = Array.isArray(data.items) ? data.items : [];
  ...
  const filteredItems = useMemo(() => {
    ...
    const filtered = items.filter((x) => {
      ...
      if (
        f.employmentType &&
        employmentType !== String(f.employmentType).toLowerCase()
      ) {
        return false;
      }
      ...
      if (
        f.experienceLevel &&
        experienceLevel !== String(f.experienceLevel).toLowerCase()
      ) {
        return false;
      }
...
function toMatchPercent(item) {
  if (item?.matchPercentage !== undefined && item?.matchPercentage !== null) {
    const value = Number(item.matchPercentage);
  ...
  // If backend sends 0.78, show 78.
  // If backend ever sends 78, do not multiply again.
