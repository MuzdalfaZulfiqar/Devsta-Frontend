...
export default function AssessmentGuard({ onViolation }) {
  ...
  // Auto-submit on violation (no alert here anymore)
  useEffect(() => {
    if (violationCount >= 1 && lastReason) {
      onViolation(lastReason);  // This calls the parent handler → shows InfoModal
    }
  }, [violationCount, lastReason, onViolation]);
