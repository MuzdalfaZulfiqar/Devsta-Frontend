...
export default function DevstaAvatar({
  ...
  // ✅ Prefer uploaded avatar, then GitHub avatar, then initials
  const photoSrc = user?.avatarUrl || user?.avatar_url || null;

  const baseClasses = [
    "relative flex items-center justify-center rounded-full font-semibold transition-all duration-200",
    colorClasses,
    showBorder && "ring-2 ring-gray-300 shadow-sm",
    hoverEffect && "hover:scale-105 hover:shadow-md",
    className,
  ]
    .filter(Boolean)
  ...
  return (
    ...
    >
      ...
      <span
        className={`absolute inset-0 flex items-center justify-center ${photoSrc ? "hidden" : ""}`}
      >
        {initials}
      </span>
