...
// //   // challenges + activities stay the same as before
...
// //   // Load saved challenges + activities when phase opens
// //   useEffect(() => {
...
// //   // Reset when phase or roadmap changes
// //   useEffect(() => {
// //     setChallenges([]);
// //     setPracticeActivities([]);
// //   }, [roadmapId, phase.title]);
...
// //   // ✅ NEW: generate topic cards on demand
...
// //         { headers: { Authorization: `Bearer ${token}` } },
// //       );
// //       setPracticeActivities(res.data.activities || []);
// //     } catch (err) {
// //       alert(
...
// //           <div
// //             className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black transition-all duration-500 ${
// //               open
...
// //           <div>
// //             <h4
// //               className={`text-lg font-black tracking-tight transition-colors ${open ? "text-gray-900 dark:text-white" : "text-gray-500"}`}
// //             >
// //               {phase.title}
// //             </h4>
...
// //                                 <span
// //                                   key={idx}
// //                                   className="text-[10px] font-bold text-indigo-500/60 uppercase"
// //                                 >
// //                                   💡 {tip}
// //                                 </span>
// //                               ))}
...
// function PhaseCard({ phase, index, role, company, userProfile, roadmapId, roadmap, onSolveChallenge, refreshKey }) {
//   const [open, setOpen] = useState(index === 0);
//   const [topicCards, setTopicCards] = useState([]);
//   const [loadingCards, setLoadingCards] = useState(false);
//   const [expandedTopic, setExpandedTopic] = useState(null);
...
//   const phaseText = (phase.title + " " + (phase.topics || []).join(" ")).toLowerCase();
//   const isAlgorithmic = /array|tree|graph|dynamic|algorithm|dsa|sorting|searching|recursion|sliding|binary|dfs|bfs|stack|queue|linked list|hashmap|heap|trie|dp|greedy/.test(phaseText);
...
//       badgeClass: "bg-gray-100 text-gray-500 border-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-700",
//     },
//     in_progress: {
//       label: "In Progress",
...
//           headers: { Authorization: `Bearer ${token}` }
//         }),
//         axios.get(`http://localhost:5000/api/roadmap/practice-activities/${roadmapId}`, {
//           params: { phaseTitle: phase.title },
//           headers: { Authorization: `Bearer ${token}` }
//         })
//       ]);
...
//       const res = await axios.post(
//         "http://localhost:5000/api/roadmap/topic-cards",
//         { topics: phase.topics, phaseTitle: phase.title, role, experience: userProfile?.experienceLevel || "mid", roadmapId },
//         { headers: { Authorization: `Bearer ${token}` } }
//       );
//       setTopicCards(res.data.cards || []);
//       setCardsGenerated(true);
...
//       const res = await axios.post(
//         "http://localhost:5000/api/roadmap/challenges",
//         { phase, role, company, preferred_language: selectedLang, count: 3, roadmapId },
//         { headers: { Authorization: `Bearer ${token}` } }
//       );
//       setChallenges(res.data.challenges || []);
//     } catch (err) {
...
//       setPracticeActivities(res.data.activities || []);
//     } catch (err) {
//       alert(err.response?.data?.message || "Failed to generate practice content");
//     } finally {
//       setLoadingActivities(false);
//     }
//   };
...
//   return (
//     <div className={`relative rounded-[2rem] transition-all duration-500 border ${
//       open
//         ? "bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 shadow-2xl shadow-gray-200/50 dark:shadow-none translate-x-1"
//         : "bg-transparent border-transparent hover:border-gray-100 dark:hover:border-gray-800"
//     }`}>
...
//         >
//           <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black transition-all duration-500 shrink-0 ${
//             open
//               ? "bg-primary text-white scale-110 shadow-lg shadow-primary/30"
//               : "bg-gray-100 dark:bg-gray-800 text-gray-400 group-hover:bg-gray-200"
//           }`}>
...
//             <h4 className={`text-lg font-black tracking-tight transition-colors truncate ${open ? "text-gray-900 dark:text-white" : "text-gray-500"}`}>
//               {phase.title}
//             </h4>
//             <div className="flex items-center gap-3 mt-1">
//               <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1">
//                 <BookOpen className="w-3 h-3" /> {phase.topics?.length || 0} Key Modules
//               </span>
//               {/* ✅ Status badge in header */}
//               <span className={`flex items-center gap-1.5 text-[9px] font-black px-2 py-0.5 rounded-full border uppercase tracking-wider ${currentStatus.badgeClass}`}>
...
//   {[
//     { value: "not_started", icon: "○", title: "Mark as Not Started", idle: "bg-gray-200/70 text-gray-500 dark:bg-gray-700 dark:text-gray-400 hover:bg-gray-300 dark:hover:bg-gray-600", active: "bg-gray-500 text-white shadow-sm" },
//     { value: "in_progress", icon: "↻", title: "Mark as In Progress", idle: "bg-amber-100/80 text-amber-600 dark:bg-amber-500/15 dark:text-amber-400 hover:bg-amber-200 dark:hover:bg-amber-500/25", active: "bg-amber-500 text-white shadow-sm shadow-amber-200 dark:shadow-none" },
//     { value: "done",        icon: "✓", title: "Mark as Done",        idle: "bg-emerald-100/80 text-emerald-600 dark:bg-emerald-500/15 dark:text-emerald-400 hover:bg-emerald-200 dark:hover:bg-emerald-500/25", active: "bg-emerald-500 text-white shadow-sm shadow-emerald-200 dark:shadow-none" },
//   ].map((opt) => (
//     <button
//       key={opt.value}
//       title={opt.title}
...
//             <ChevronDown className={`w-6 h-6 text-gray-300 transition-transform duration-500 ${open ? "rotate-180 text-primary" : ""}`} />
//           </button>
//         </div>
//       </div>
...
//         <span
//         className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Status:</span>
//         {[
