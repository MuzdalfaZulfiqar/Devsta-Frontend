...
// import { 
//   CheckCircle2, Code2, Sparkles, TrendingUp,
...
//   if (loading) return (
//     <div className="py-20 text-center text-gray-400">Loading your progress...</div>
//   );

//   if (!roadmaps.length) return (
//     <div className="py-20 text-center text-gray-400">
//       No roadmaps yet. Generate one first.
//     </div>
//   );
...
//         className="border rounded-xl px-4 py-2.5 text-sm dark:bg-gray-800 dark:border-gray-700 font-medium"
//       >
//         {roadmaps.map(r => (
//           <option key={r._id} value={r._id}>{r.role} — {r.experience}</option>
//         ))}
//       </select>
...
//             {[
//               { label: "Solved", value: stats.solved, icon: <CheckCircle2 className="w-5 h-5 text-emerald-500" />, color: "text-emerald-600 dark:text-emerald-400" },
//               { label: "Attempted", value: stats.attempted, icon: <Code2 className="w-5 h-5 text-orange-500" />, color: "text-orange-600 dark:text-orange-400" },
//               { label: "AI Reviewed", value: stats.reviewed, icon: <Sparkles className="w-5 h-5 text-violet-500" />, color: "text-violet-600 dark:text-violet-400" },
//               { label: "Avg Score", value: `${Math.round(stats.avgScore)}%`, icon: <TrendingUp className="w-5 h-5 text-blue-500" />, color: "text-blue-600 dark:text-blue-400" },
//             ].map(s => (
...
//             <h3 className="text-sm font-black uppercase tracking-widest text-gray-400">Progress by Phase</h3>
//             {Object.entries(stats.byPhase).map(([phase, data]) => (
//               <div key={phase}>
//                 <div className="flex justify-between mb-1">
//                   <span className="text-xs font-semibold text-gray-700 dark:text-gray-300 truncate max-w-xs">{phase}</span>
//                   <span className="text-xs text-gray-400">{data.solved}/{data.total}</span>
//                 </div>
...
//           </div>
//           <div>
//             <h3 className="text-sm font-black text-gray-900 dark:text-white">Phase Progress</h3>
//             <p className="text-[10px] text-gray-400 uppercase tracking-widest">
//               {doneCount}/{phases.length} phases completed
//             </p>
//           </div>
//         </div>
//         <span className={`text-2xl font-black tabular-nums ${
//           overallPct === 100 ? "text-emerald-500" : overallPct >= 50 ? "text-amber-500" : "text-primary"
//         }`}>
//           {overallPct}%
//         </span>
//       </div>
...
//           const config = {
//             not_started: { label: "Not started", dot: "bg-gray-400",    bar: "bg-gray-300",    pct: 0   },
//             in_progress:  { label: "In progress",  dot: "bg-amber-400 animate-pulse", bar: "bg-amber-400",  pct: 50  },
//             done:         { label: "Done",          dot: "bg-emerald-500", bar: "bg-emerald-500", pct: 100 },
//           }[st];

//           return (
//             <div key={i} className="flex items-center gap-3">
//               {/* Phase number */}
//               <span className="w-6 h-6 rounded-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-[10px] font-black text-gray-500 shrink-0">
//                 {i + 1}
//               </span>
//               {/* Title */}
//               <span className="flex-1 text-xs font-semibold text-gray-700 dark:text-gray-300 truncate min-w-0">
//                 {phase.title}
//               </span>
//               {/* Mini progress bar */}
//               <div className="w-20 h-1.5 rounded-full bg-gray-100 dark:bg-gray-800 overflow-hidden shrink-0">
//                 <div
...
//               </div>
//               {/* Status pill */}
//               <span className={`flex items-center gap-1 text-[9px] font-black px-2 py-0.5 rounded-full shrink-0 ${
//                 st === "done"        ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400" :
//                 st === "in_progress" ? "bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400" :
...
//         {[
//           { dot: "bg-gray-400",    label: `${phases.filter(p => (phaseProgress[p.title] || "not_started") === "not_started").length} not started` },
//           { dot: "bg-amber-400",   label: `${inProgressCount} in progress` },
//           { dot: "bg-emerald-500", label: `${doneCount} done` },
//         ].map(({ dot, label }) => (
...
//           </div>
//           <div>
//             <h3 className="text-sm font-black text-gray-900 dark:text-white">Submission History</h3>
//             <p className="text-[10px] text-gray-400 uppercase tracking-widest">All your past solutions & reviews</p>
//           </div>
//         </div>

//         {submissions.length === 0 ? (
//           <div className="py-10 text-center text-gray-400 text-sm">
//             No submissions yet. Solve a challenge to see your history here.
//           </div>
//         ) : (
//           <div className="space-y-3">
//             {submissions.map((sub) => (
//               <div key={sub._id} className="rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
...
//                         {sub.challenge?.phaseTitle} · {sub.language} · {new Date(sub.submittedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" })}
//                       </p>
//                     </div>
//                   </div>
...
//                         sub.score === 100 ? "text-emerald-500" :
...
//                     {sub.code && (
//                       <div>
//                         <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-4 pt-3 pb-1 flex items-center gap-1.5">
//                           <Code2 className="w-3 h-3" /> {sub.language} solution
//                         </p>
//                         <pre className="px-4 pb-4 text-[11px] font-mono text-gray-300 bg-gray-950 overflow-auto max-h-52 leading-relaxed">
//                           {sub.code}
//                         </pre>
//                       </div>
//                     )}
...
//                     {/* AI Review */}
//                     {sub.aiReview && (
//                       <div className="border-t border-gray-200 dark:border-gray-800 px-4 py-4 bg-violet-50/30 dark:bg-violet-500/5 space-y-3">
...
//                         </p>
//                         {/* Parse sections */}
//                         {sub.aiReview.split(/###\s+\d+\.\s+/).filter(Boolean).map((section, i) => {
...
//                               <p className={`text-[10px] font-bold uppercase tracking-wider ${colors[i % colors.length]}`}>{heading}</p>
//                               <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">
//                                 {body.replace(/^[-*]\s*/gm, "").replace(/\*\*(.*?)\*\*/g, "$1").trim()}
//                               </p>
//                             </div>
//                           );
//                         })}
//                         {/* Score */}
//                         {sub.aiReview.match(/\*\*Final Score:\s*(\d+)\/100\*\*/) && (() => {
//                           const score = parseInt(sub.aiReview.match(/\*\*Final Score:\s*(\d+)\/100\*\*/)[1]);
//                           const color = score >= 80 ? "text-emerald-500" : score >= 60 ? "text-amber-500" : "text-rose-500";
//                           return (
//                             <p className={`text-sm font-black ${color}`}>Final Score: {score}/100</p>
//                           );
//                         })()}
//                       </div>
//                     )}
...
//   // Very simple token colorizer — keywords, strings, numbers, comments
//   const colorize = (line) => {
//     const keywords = /\b(def|return|if|else|elif|for|while|in|import|from|class|True|False|None|and|or|not|try|except|with|as|pass|break|continue|lambda|yield|async|await|const|let|var|function|arrow|=>|new|this|typeof|instanceof|null|undefined|int|str|list|dict|bool|print|range|len)\b/g;
//     const strings = /(["'`])(?:(?!\1)[^\\]|\\.)*\1/g;
...
//       .replace(/</g, "&lt;")
//       .replace(/>/g, "&gt;")
//       .replace(comments, '<span class="text-gray-500 italic">$&</span>')
//       .replace(strings, '<span class="text-emerald-400">$&</span>')
//       .replace(numbers, '<span class="text-amber-400">$&</span>')
//       .replace(keywords, '<span class="text-violet-400 font-semibold">$&</span>');
//   };
...
//           </div>
//           <span className="text-[10px] font-mono text-gray-400 ml-2 uppercase tracking-widest">{language}</span>
//         </div>
//         <button
//           onClick={copy}
//           className="text-[10px] font-bold text-gray-400 hover:text-white transition-colors px-2 py-1 rounded-lg hover:bg-gray-700"
//         >
//           {copied ? "✓ Copied" : "Copy"}
//         </button>
//       </div>
//       {/* Code body */}
//       <div className="bg-gray-950 overflow-auto max-h-56">
//         <table className="w-full border-collapse text-[11px] font-mono leading-relaxed">
//           <tbody>
//             {code.split("\n").map((line, i) => (
//               <tr key={i} className="hover:bg-white/5 transition-colors">
//                 <td className="select-none text-right pr-4 pl-4 py-0.5 text-gray-600 border-r border-gray-800 w-10 shrink-0">
//                   {i + 1}
//                 </td>
//                 <td
//                   className="px-4 py-0.5 text-gray-300 whitespace-pre"
...
//   // ✅ Filter submissions to only those belonging to this roadmap's challenges
...
//                     : "bg-white dark:bg-gray-900 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-primary/40 hover:text-primary"
//                 }`}
//               >
//                 <span className="truncate max-w-[160px] block">{r.role}</span>
//                 <span className={`text-[10px] uppercase tracking-wider font-black ${selectedId === r._id ? "text-white/70" : "text-gray-400"}`}>
//                   {r.experience}
//                 </span>
//               </button>
//             ))}
...
//                 return (
//                   <div key={sub._id} className="overflow-hidden">
//                     <button
//                       onClick={() => setExpandedSub(expandedSub === sub._id ? null : sub._id)}
//                       className="w-full flex items-center gap-4 px-6 py-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition text-left"
//                     >
...
//                         {sub.aiReview && (
//                           <button
//                             onClick={(e) => { e.stopPropagation(); setActiveReview(sub.aiReview); }}
...
//                           </button>
//                         )}
//                         {expandedSub === sub._id ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
//                       </div>
//                     </button>

//                     {/* Expanded: show code only (review opens in modal) */}
//                     {expandedSub === sub._id && sub.code && (
//                       <div className="border-t border-gray-100 dark:border-gray-800 px-6 py-4 bg-gray-950">
//                         <CodeBlock code={sub.code} language={sub.language || "code"} />
//                       </div>
//                     )}
//                   </div>
//                 );
