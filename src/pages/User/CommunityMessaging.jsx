  ...
  const handleSend = async ({ text, file }) => {
    ...
    try {
      ...
      if (isImage || isVideo) {
        ...
        // CASE 3: Document (PDF/Word/Excel/PPT/other) -> local attachment API
        const newMsg = await sendFileMessage(
          selectedConversation._id,
          file,
          trimmed
        );
        // add to current message list so sender sees it immediately
        setMessages((prev) => [...prev, newMsg]);
        // NOTE: Receiver will see it when they reload / re-open the convo
        // If you want real-time for docs too, we can later emit via socket from the backend in /with-file.
      }
    } catch (err) {
      console.error("Error sending message:", err);
      // TODO: show toast
