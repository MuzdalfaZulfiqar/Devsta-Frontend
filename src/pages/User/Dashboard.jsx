...
export default function Dashboard() {
  ...
  // ── Keep dashboard user fresh after Module 3 skill sync ─────
  useEffect(() => {
    if (!token) return;
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setUser(freshUser.user ?? freshUser);
      } catch (error) {
        console.warn("Failed to refresh dashboard user:", error);
      }
    })();
  ...
  const displayUser = {
    ...user,
    topSkills: mergeSkillLists(user?.topSkills || [], user?.acquiredSkills || []),
  };
  ...
  const validateSkills = async () => {
    const sourcesCount =
      (displayUser.topSkills?.length > 0 ? 1 : 0) +
      (user.githubConnected ? 1 : 0) +
      (user.resumeUrl ? 1 : 0) +
      (user.hasAttemptedQuiz ? 1 : 0);
  ...
  const sourcesAvailable = [
    displayUser?.topSkills?.length > 0,
    user?.githubConnected,
    !!user?.resumeUrl,
    user.hasAttemptedQuiz,
  ];
  ...
  return (
    <DashboardLayout user={displayUser}>
      <div className="flex flex-col gap-8 w-full">
        ...
        {/* ── Validated Skills ── */}
        {(displayUser.skillsValidated || allSourcesAvailable) && (
          <div id="validated-skills-section" className="mt-8">
            <ValidatedSkills
              validatedSkills={displayUser.validatedSkills}
              profileScore={displayUser.profileScore}
              user={displayUser}
              onValidate={validateSkills}
              isValidating={isValidating}
            />
