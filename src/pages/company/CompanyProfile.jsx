...
// src/pages/company/CompanyProfile.jsx
// ── CHANGE from original: Security section now links to /company/forgot-password
import { useState, useEffect, useRef } from "react";
...
import { getCompanyProfile, updateCompanyProfile } from "../../api/company/profile";
import { showToast } from "../../utils/toast";
import { Building2, Mail, Globe, BriefcaseBusiness, Save, X, Upload, Calendar, FileText, KeyRound } from "lucide-react";
      ...
      <div className="max-w-5xl mx-auto py-8 px-4 sm:px-6">
        ...
        <div className="bg-white rounded-xl shadow border overflow-hidden">
          ...
          <div className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-medium text-gray-500 mb-1.5">Industry / Focus</label>
                  {isEditing ? (
                    <input
                      type="text"
                      name="industry"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-500 mb-1.5">Founded Year</label>
                  {isEditing ? (
                    <input
                      type="number"
                      name="foundedYear"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-500 mb-1.5">Company Size</label>
                  {isEditing ? (
                    <select
                      name="companySize"
                      value={formData.companySize}
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-medium text-gray-500 mb-1.5">Website</label>
                  {isEditing ? (
                    <input
                      type="url"
                      name="website"
                    ...
                    <div className="text-gray-900 font-medium">
                      {profile.website ? (
                        <a href={profile.website} target="_blank" rel="noopener noreferrer" className="text-[#086972] hover:underline">
                          {profile.website}
                        </a>
                      ) : "—"}
                    </div>
                ...
                <div className="col-span-full">
                  <label className="block text-sm font-medium text-gray-500 mb-1.5">Company Summary / About</label>
                  {isEditing ? (
                    <textarea
                      name="description"
                      value={formData.description}
        ...
        <div className="mt-10 bg-white rounded-xl shadow border p-8">
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 rounded-lg bg-[#086972]/10 flex items-center justify-center flex-shrink-0 mt-0.5">
              <KeyRound size={20} className="text-[#086972]" />
            </div>
            <div className="flex-1">
              <h3 className="text-xl font-semibold mb-1">Password & Security</h3>
              <p className="text-gray-500 text-sm mb-6 leading-relaxed">
                Reset your password securely using your registered email address. We'll send you a one-time code to verify it's you.
              </p>
              <button
                onClick={() => navigate("/company/forgot-password")}
                className="px-6 py-2.5 bg-[#086972] text-white rounded-lg hover:bg-[#086972]/90 transition font-medium flex items-center gap-2"
              >
                <KeyRound size={16} />
                Change Password
              </button>
