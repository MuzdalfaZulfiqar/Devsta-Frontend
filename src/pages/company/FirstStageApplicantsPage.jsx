      ...
      <div className="bg-white dark:bg-gray-900 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-1 gap-4">
            <div>
              ...
              <p className="mt-1 text-xs text-gray-400">
                Leave empty — link activates immediately
              </p>
            </div>
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              onClick={() =>
                onConfirm({ availableFrom: start || undefined, availableUntil: end })
              }
...
function TestStatusBadge({ assessment }) {
  ...
  const map = {
    invited: {
      label: "Invited",
      cls: "bg-blue-50 text-blue-600 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400",
      Icon: Mail,
    },
    in_progress: {
      label: "In Progress",
      cls: "bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400",
      Icon: Clock,
    },
    submitted: {
      label: "Submitted",
      cls: "bg-green-50 text-green-600 border-green-200 dark:bg-green-900/30 dark:text-green-400",
      Icon: ClipboardCheck,
    },
    evaluated: {
      label: "Evaluated",
      cls: "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400",
      Icon: Star,
    },
  };
  ...
  return (
    <div className="flex flex-col gap-0.5">
      <span
        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium border ${config.cls}`}
      >
        <Icon size={11} />
        {config.label}
      </span>
...
export default function FirstStageApplicantsPage() {
  ...
  const [draftApplicants, setDraftApplicants] = useState([]); // applicationIds as strings
  ...
  const confirmSendTestToOne = async ({ availableFrom, availableUntil }) => {
    ...
    if (end <= now) {
      showToast("Expiry date must be in the future", 5000);
      return;
    }
    if (start && start >= end) {
      showToast("Start must be before expiry", 5000);
      return;
    }
  ...
  const refreshDraftSelection = async () => {
    try {
      ...
      setDraftApplicants(
        (data.draftSelectedApplicants || []).map((app) => String(app._id))
      );
    } catch (err) {
  ...
  const confirmShortlist = async () => {
    ...
    try {
      ...
      showToast(data.message || `Shortlisted candidates!`, 5000);
...
setCodingSessions({}); 
  ...
  const uninvitedApplicants = useMemo(() => {
    ...
    return applicants.filter((a) => {
      ...
      if (
        a.assessment?.expiresAt &&
        new Date(a.assessment.expiresAt) < new Date() &&
        !["submitted", "evaluated"].includes(s)
      )
        return true;
      return false;
    });
  ...
  const eligibleDraftCount = useMemo(
    () =>
      draftApplicants.filter((id) =>
        uninvitedApplicants.some((a) => String(a._id) === id)
      ).length,
...
useEffect(() => {
  ...
  const confirmMoveToFinal = async () => {
    ...
    try {
      ...
      showToast(data.message || "Moved candidates to final stage!", 5000);
  ...
  const confirmFinalDecision = async () => {
    ...
    try {
      ...
      showToast(err.message || "Failed to make decision", 5000);
    }
  };
                ...
                <button
                  onClick={async () => {
                    try {
                      ...
                      await addDraftApplicants(
                        jobId,
                        uninvitedApplicants.map((a) => String(a._id))
                      );
                      await refreshDraftSelection();
                      showToast(`Selected ${uninvitedApplicants.length} uninvited candidate(s)`, 3000);
                    } catch {
                      showToast("Failed to select uninvited", 5000);
                    }
                  }}
          ...
          <div className="mb-6">
            {job?.selectedChallenges?.length > 0 ? (
              ...
              >
                ...
                {eligibleDraftCount > 0 && (
                  <span className="ml-0.5 opacity-80">({eligibleDraftCount} eligible)</span>
                )}
              </Link>
            )}
          ...
          {activeTab === "assessment" && (
            <>
              ...
              >
                {shortlisting ? (
                  <>
                    <Loader2 size={14} className="animate-spin" /> Shortlisting…
                  </>
                ) : (
                  `Shortlist (${draftCount})`
                )}
              </button>
            </>
          )}
              ...
              return (
                <section key={key}>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span
                      className={`text-sm font-medium px-2.5 py-0.5 rounded-full bg-${color}-100 dark:bg-${color}-900/30 text-${color}-700 dark:text-${color}-400`}
                    >
                      {group.length}
                    </span>
                  </div>
                  ...
                  {group.length === 0 ? (
                    <div
                      className={`p-8 text-center rounded-xl border border-${color}-200 dark:border-${color}-800 bg-${color}-50/40 dark:bg-${color}-950/20 text-${color}-600 dark:text-${color}-400 text-sm`}
                    >
                      None yet.
                    </div>
                  ) : (
                    <div className="bg-white dark:bg-gray-800 rounded-xl border overflow-hidden">
                      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-900/60">
                          <tr>
                            {["Name", "Email", "Test Score", "Interview Score", ...(key === "final_review" ? ["Actions"] : [])].map(
                              (h) => (
                                <th
                                  key={h}
                  ...
                  {filteredApplicants.map((app) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Name / Email / Phone */}
                        <td className="px-5 py-3.5 whitespace-nowrap font-medium text-gray-900 dark:text-gray-100 text-sm">
                          {dev.name || "—"}
                        </td>
                            ...
                            <td className="px-5 py-3.5 whitespace-nowrap">
                              {ml ? (
                                <div className="flex flex-wrap gap-1.5">
                                  ...
                                  <ScorePill label="Resume" value={ml.resumeMatch} />
                            ...
                            <td className="px-5 py-3.5 whitespace-nowrap">
                              {dev.resumeUrl ? (
                                <div className="flex gap-2">
                                  ...
                                  <button
                                    onClick={() => handleDownloadResume(dev._id)}
                                    className="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-600 hover:bg-gray-200 transition"
                                  >
                                    <Download size={12} /> DL
                                  </button>
                                </div>
...
function ScorePill({ label, value, strong = false }) {
  ...
  // Scores in your app are 0–100, so use 80/60 thresholds (not 0.80/0.60)
  if (Number.isFinite(v)) {
    if (v >= 80) colorClass = "bg-green-100 text-green-700 border-green-200";
    else if (v >= 60) colorClass = "bg-yellow-100 text-yellow-700 border-yellow-200";
