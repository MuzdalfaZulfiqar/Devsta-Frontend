...
export default function CompanyCodingChallengePage() {
  ...
  // Draft selection count from backend
  ...
  // Track if test config is saved (from backend job)
  const [isConfigSaved, setIsConfigSaved] = useState(false);
  // Send test confirmation
  const [showSendConfirm, setShowSendConfirm] = useState(false);

  // Near other states (e.g. after const [showSendConfirm, setShowSendConfirm] = useState(false);)
  const [testDurationDays, setTestDurationDays] = useState(7);
  const [testStart, setTestStart] = useState("");     // ISO string like "2026-03-10T09:00"
  const [testEnd, setTestEnd] = useState("");

  const [sendErrors, setSendErrors] = useState([]);

  const [isSending, setIsSending] = useState(false);   // ← NEW: controls loader on send button
  // Add this state
  ...
  // Selected challenges for preview
  const selectedChallenges = useMemo(
    () =>
      selectedIds
        .map((id) => acceptedChallenges.find((c) => c._id === id))
  ...
  //     const ids = draftData.draftSelectedApplicants.map((app) => app._id);

  //     const results = await Promise.allSettled(
  //       ids.map((id) => sendTestToApplicant(jobId, id))
  //     );
  ...
  //     if (success > 0) {
  //       showToast(`Test sent to ${success} candidate(s)!`, "success");
  //     }
  //     if (failed > 0) {
  //       showToast(`${failed} failed — please retry`, "error");
  //     }
  ...
  //     fetchJob();
  //   } catch (err) {
  //     showToast("Failed to send tests", "error");
  //     console.error(err);
  //   } finally {
  ...
  const confirmSendTest = async () => {
    ...
    const startDate = testStart ? new Date(testStart) : null;
    const endDate = new Date(testEnd);
    const now = new Date();
    ...
    try {
      ...
      if (ids.length === 0) {
        showToast("No candidates selected to send test to", "warning");
        return;
      }

      const payload = {
        availableFrom: testStart || undefined,
        availableUntil: testEnd,
      };

      showToast(`Sending test invitations to ${ids.length} candidate${ids.length !== 1 ? 's' : ''}…`, "info");

      const results = await Promise.allSettled(
        ids.map((id) => sendTestToApplicant(jobId, id, payload))
      );
      ...
      if (successCount > 0) {
        showToast(`Successfully sent to ${successCount} candidate${successCount !== 1 ? 's' : ''}`, "success");
      }

      const uniqueErrors = [...new Set(errorMessages)];

      if (uniqueErrors.length > 0) {
        setSendErrors(uniqueErrors);
        return; // keep modal open to show errors
      }

      // Success path: clear everything
      await clearDraftApplicants(jobId);
      setDraftCount(0);
      setTestStart("");
      setTestEnd("");
      setIsDrawerOpen(false);
      fetchJob();
      await fetchDraftCount();           // ← add
     
      setShowSendConfirm(false); // close modal only on full success

    } catch (err) {
      console.error("Bulk send failed:", err);
      setSendErrors(["Unexpected error: " + (err.message || "Please try again")]);
    } finally {
      setIsSending(false);   // ← Always stop loader
    }
  };
  ...
  const fetchDraftCount = async () => {
    try {
      ...
      showToast("Could not load selected candidates count", 5000);
    }
  };

  // ─────────────────────────────────────────────
  // Fetch job & check config status
  // ─────────────────────────────────────────────
  const fetchJob = async () => {
    if (!token) return;
    setLoadingJob(true);
    try {
      const data = await getJobById(jobId);
      setJob(data);
      // Config is saved only if there are selected challenges
      setIsConfigSaved(!!data.selectedChallenges?.length);
    } catch (err) {
      showToast(err.message || "Failed to load job details", 4000);
    } finally {
      setLoadingJob(false);
  ...
  // Pre-fill selected config from job
  useEffect(() => {
    if (job?.selectedChallenges) {
      const ids = job.selectedChallenges.map((s) => s.challengeId.toString());
      setSelectedIds(ids);
  ...
  //     // Refetch job to confirm config is saved
  //     await fetchJob();
  //   } catch (err) {
  //     showToast(err.message || "Failed to save configuration.", 4000);
  //   }
  ...
  const handleSaveTestConfig = async () => {
    if (selectedIds.length === 0) { showToast("Select at least one challenge.", 4000); return; }
    if (totalTimeOverride > 0 && totalTimeOverride < realSumOfTimes) {
      showToast(`Total time too low. Increase or set to 0 (auto).`, 7000); return;
    }
    setIsSaving(true);
    try {
      const selected = selectedIds.map((id, index) => {
        const challenge = acceptedChallenges.find((c) => c._id === id);
        return { challengeId: id, timeLimit: challengeTimes[id] || challenge?.timeLimit || 30, order: index };
      });
      await saveJobTestConfig(jobId, { selectedChallenges: selected, testTotalTime: totalTimeOverride || 0 });
      showToast("Test configuration saved!", 4000);
      navigate(`/company/jobs/${jobId}/first-stage-applicants`); // ← redirect back
          ...
          <div className="mt-6 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-amber-800 dark:text-amber-300">
                This job already has a configured test
              </p>
              <p className="text-xs text-amber-700 dark:text-amber-400 mt-0.5">
                You're reconfiguring. Candidates already invited keep their original test.
                Only new invitations will use the updated config.
              </p>
            </div>
          </div>
        )} */}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-medium text-amber-800 text-base">
                  Test already sent to some candidates
                </p>
                <p className="text-sm text-amber-700 mt-1 leading-relaxed">
                  Any changes you make now (adding new challenges, changing order, time limits)
                  <strong> only affect future candidates</strong>.
                  <br /><br />
                  People who already received or started the test will keep seeing the original version.
                  <br />
                  <strong>Avoid heavy edits</strong> to already-used challenges — it can make scoring unfair.
                </p>
              </div>
            </div>
          </div>
        ...
        {!loading && challenges.length > 0 && (
          <div className="border-b border-gray-200 dark:border-gray-700 mb-6 overflow-x-auto">
            <nav className="flex space-x-8 min-w-max" aria-label="Tabs">
              <button
                onClick={() => setActiveTab("pending")}
                className={`py-4 px-1 font-medium text-sm border-b-2 transition-all ${activeTab === "pending"
                  ? "border-blue-600 text-blue-600"
                  : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-200 dark:hover:border-gray-600"
                  }`}
              >
              ...
              <button
                onClick={() => setActiveTab("accepted")}
                className={`py-4 px-1 font-medium text-sm border-b-2 transition-all ${activeTab === "accepted"
                  ? "border-green-600 text-green-600"
                  : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-200 dark:hover:border-gray-600"
                  }`}
              >
              ...
              <button
                onClick={() => setActiveTab("rejected")}
                className={`py-4 px-1 font-medium text-sm border-b-2 transition-all ${activeTab === "rejected"
                  ? "border-red-600 text-red-600"
                  : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-200 dark:hover:border-gray-600"
                  }`}
              >
          ...
          <div className="mb-10 p-6 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
            ...
            <div className="flex flex-wrap gap-4">
              {/* <button
                onClick={handleSaveTestConfig}
                disabled={selectedIds.length === 0}
                className={`px-6 py-2.5 rounded-lg text-white font-medium shadow-sm min-w-[180px] ${selectedIds.length === 0
                  ? "bg-purple-400 cursor-not-allowed"
                  : "bg-purple-600 hover:bg-purple-700"
                  }`}
              >
              ...
              <button onClick={handleSaveTestConfig} disabled={selectedIds.length === 0 || isSaving}
                className={`px-5 py-2.5 rounded-lg text-white font-medium flex items-center gap-2 transition
    ${selectedIds.length === 0 || isSaving ? "bg-purple-400 cursor-not-allowed" : "bg-purple-600 hover:bg-purple-700"}`}>
                {isSaving
                  ? <>Saving…</>
                  : <><CheckCircle2 size={16} />{isConfigSaved ? "Update Configuration" : "Save Configuration"}</>}
              </button>
              <button onClick={() => navigate(`/company/jobs/${jobId}/first-stage-applicants`)}
                className="px-5 py-2.5 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 flex items-center gap-2 transition">
              ...
              <button
                onClick={() => setIsDrawerOpen(true)}
                disabled={selectedIds.length === 0}
                className={`px-6 py-2.5 rounded-lg text-white font-medium shadow-sm min-w-[220px] ${selectedIds.length === 0
                  ? "bg-gray-400 cursor-not-allowed"
                  : "bg-indigo-600 hover:bg-indigo-700"
                  }`}
              >
        ...
        {/* Ready to Send Section – Gated by config */}
        <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
          <div className="mb-4">
            <h4 className="font-medium text-lg text-gray-900 dark:text-white">
              Ready to invite candidates?
            </h4>
            {eligibleDraftCount === 0 && draftCount > 0 && (
              <p className="mt-2 text-sm text-amber-700 dark:text-amber-400 text-center">
                All selected candidates have already been invited (or status changed).
              </p>
            )}
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {draftCount === 0
                ? "No eligible candidates selected yet. Go back to applicants to choose who should receive this test."
                : `This test will be sent to ${draftCount} selected candidate${draftCount !== 1 ? 's' : ''}.`}
            </p>

            {/* Config warning if candidates selected but no config saved */}
            {draftCount > 0 && !isConfigSaved && (
              <div className="mt-3 p-3 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 rounded-lg text-yellow-800 dark:text-yellow-200 text-sm flex items-start gap-2">
                <AlertTriangle size={18} className="mt-0.5 flex-shrink-0" />
                <span>
                  No coding challenges configured yet. Go to the "Accepted" tab, select challenges, and click "Save Configuration" before sending tests.
                </span>
              </div>
            )}
          </div>
          ...
          {/* <button
            ...
            //   if (!isConfigSaved) {
            //     showToast("Please save test configuration first (Accepted tab)", "warning");
            //     return;
            //   }

            //   if (!window.confirm(`Send test to ${draftCount} candidate${draftCount !== 1 ? 's' : ''}?`)) {
            //     return;
            //   }
            ...
            //   }
            // }}
            // disabled={draftCount === 0 || !isConfigSaved}
            // className={`w-full flex items-center justify-center gap-2 px-6 py-3 font-medium rounded-lg transition shadow-sm ${draftCount === 0 || !isConfigSaved
            //     ? "bg-gray-400 text-gray-700 cursor-not-allowed"
            //     : "bg-green-600 hover:bg-green-700 text-white"
            //   }`}
            // onClick={() => {
            //   if (draftCount === 0) {
            //     showToast("No candidates selected", 5000);
            //     return;
            //   }

            //   if (!isConfigSaved) {
            //     showToast("Please save test configuration first (Accepted tab)", 5000);
            //     return;
            //   }

            //   setShowSendConfirm(true); // 🔥 open modal instead of window.confirm
            // }}

            // Replace the button label/disabled logic:
            onClick={() => {
              if (draftCount === 0) { showToast("No candidates selected", 5000); return; }
              if (!isConfigSaved) { showToast("Please save test configuration first", 5000); return; }
              setShowSendConfirm(true);
            }}
          >
            <Send size={18} />
            Send Test to {draftCount} Candidate{draftCount !== 1 ? 's' : ''}
          </button> */}
       <button
  onClick={() => {
    if (draftCount === 0) {
    ...
    if (!isConfigSaved) {
      showToast("Please save test configuration first", 5000);
      return;
    }
    setShowSendConfirm(true);
  }}
  disabled={draftCount === 0 || !isConfigSaved || isSending}
  className={`w-full flex items-center justify-center gap-2 px-6 py-3 font-medium rounded-lg transition shadow-sm
    ${draftCount === 0 || !isConfigSaved
      ? "bg-gray-400 text-gray-700 cursor-not-allowed"
      : "bg-green-600 hover:bg-green-700 text-white"}`}
>
  ...
  Send Test to {draftCount} Candidate{draftCount !== 1 ? 's' : ''}
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to send this test to ${draftCount} candidate${draftCount !== 1 ? 's' : ''}? This action cannot be undone.`}
      ...
      <ConfirmModal
        ...
        message={
          <div className="space-y-6 text-left">
            <p className="font-medium">
              You are about to send the coding test to <strong>{draftCount} candidate{draftCount !== 1 ? 's' : ''}</strong>.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div>
                ...
                />
                <p className="mt-1 text-xs text-gray-500">Leave empty = immediately after invite</p>
              </div>
            ...
            {sendErrors.length > 0 && (
              <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg text-sm text-red-800">
                <p className="font-medium mb-2">Some invitations could not be sent:</p>
                <ul className="list-disc pl-5 space-y-1.5 max-h-48 overflow-y-auto">
                  {sendErrors.map((err, index) => (
                ...
                <p className="mt-3 text-xs opacity-80">
                  You can correct the dates and try again, or send to remaining candidates.
                </p>
              </div>
            )}

            <p className="text-sm text-gray-600 dark:text-gray-400">
              Candidates can only <strong>start</strong> the test within the selected window.
              The invitation link will become invalid after the expiry date.
            </p>
          </div>
        }
        confirmLabel={
        ...
        cancelLabel="Cancel"
        disabledConfirm={isSending}   // ← disable button while sending
        onConfirm={() => {
          if (sendErrors.length > 0) {
            setSendErrors([]); // clear previous errors
          }
          confirmSendTest();
        }}
        onCancel={() => {
      ...
      {/* Selected Challenges Drawer */}
      <div
        className={`fixed top-0 right-0 h-full w-96 bg-white dark:bg-gray-800 shadow-2xl transform transition-transform duration-300 ease-in-out z-40 ${isDrawerOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
